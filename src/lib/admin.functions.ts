...
export const adminUpdateTenant = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    z.object({
      ...
      move_in_date_bs: z.string().trim().regex(/^(207\d|208\d|209\d|2100)-(0?[1-9]|1[0-2])-(0?[1-9]|[12]\d|3[0-2])$/, "BS date must be YYYY-MM-DD").nullable().optional().or(z.literal("").transform(() => null)),
