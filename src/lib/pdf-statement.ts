...
// ─── Exact BS day ─────────────────────────────────────────────────────────────
// Each entry: [adYear, adMonth, adDay, bsYear, bsMonth] — the AD date that
// starts that BS month (day 1). We keep 2082–2084 which covers the near future.
...
function getTodayBSFull(): { year: number; month: number; day: number } {
  const bs = approxCurrentBS(); // gives year + month
  const now = new Date();
  const adYear = now.getFullYear(), adMonth = now.getMonth() + 1, adDay = now.getDate();

  // Find the entry for the current BS year+month → get its AD start day
  const entry = BS_MONTH_START_DAYS.find(([, , , y, m]) => y === bs.year && m === bs.month);
  if (!entry) return { ...bs, day: adDay }; // fallback: use AD day

  const [startAdY, startAdM, startAdD] = entry;

  // Days elapsed since the AD date that started this BS month
  const startDate = new Date(startAdY, startAdM - 1, startAdD);
...
export async function generateStatement(
  ...
  const drawHeader = (doc: jsPDF) => {
    ...
    doc.text("Hamro Rent  ·  Landlord's Auto Calculator", logoDataUrl ? ML + 30 : ML, 21);
  ...
  const drawFooter = (doc: jsPDF, pageNum: number, totalPages: number) => {
    ...
    const now = new Date();
    const adDate = now.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
    const bsFull = getTodayBSFull();
    const bsDateStr = `${bsFull.year}-${String(bsFull.month).padStart(2,"0")}-${String(bsFull.day).padStart(2,"0")}`;
    doc.text(
      `Generated by Hamro Rent  ·  hamrorent.app  ·  ${bsDateStr} BS  |  ${adDate} AD`,
      ML, PH - 7
    );
  ...
  const drawTitle = (doc: jsPDF, y: number): number => {
    ...
    const bsFull = getTodayBSFull();
    doc.text(
      `Generated: ${bsFull.year}-${String(bsFull.month).padStart(2,"0")}-${String(bsFull.day).padStart(2,"0")} BS  |  ${adDate} AD`,
      PW - MR - 4, y + 13, { align: "right" }
    );
