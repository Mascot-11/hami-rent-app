      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Button
            ...
            onClick={async () => {
              if (!canShare) {
                toast.error("Bill sharing is a Pro/Basic feature. Upgrade to share bills with tenants.");
                return;
              }
              try {
            ...
            className={`text-xs sm:text-sm ${!canShare ? "opacity-50 cursor-not-allowed" : ""}`}
            title={!canShare ? "Upgrade to Basic or Pro to share bills" : undefined}
          >
            <Share2 className="h-4 w-4 mr-1" />
            <span className="hidden sm:inline">Share link</span>
            <span className="sm:hidden">Share</span>
            {!canShare && <span className="ml-1 text-[10px] bg-muted px-1 rounded">PRO</span>}
          </Button>
          ...
          <Button
            ...
            onClick={() => {
              if (!canShare) {
                toast.error("Bill sharing is a Pro/Basic feature. Upgrade to share bills via WhatsApp.");
                return;
              }
              const token = bill.share_token;
              if (!token) { toast.error("Share token not found"); return; }
              ...
              const message = encodeURIComponent(`Hi ${bill.tenants?.name ?? "there"}, your bill for ${bsMonthName(bill.bs_month)} ${bill.bs_year} is *${fmtNPR(total)}*${remaining > 0 ? ` (remaining: *${fmtNPR(remaining)}*)` : " ✅ Fully paid"}.\nView details: ${url}`);
              const waUrl = phone
                ? `https://wa.me/${phone}?text=${message}`
                : `https://wa.me/?text=${message}`;
              window.open(waUrl, "_blank");
            }}
            className={`text-xs sm:text-sm text-green-600 border-green-600 hover:bg-green-50 ${!canShare ? "opacity-50 cursor-not-allowed" : ""}`}
            title={!canShare ? "Upgrade to Basic or Pro to share via WhatsApp" : undefined}
          >
