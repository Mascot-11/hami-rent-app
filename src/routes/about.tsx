  ...
  head: () => ({
    meta: [
      { title: "About Hamro Rent — Built in Nepal for Nepali Landlords" },
      {
        name: "description",
        content:
          "Hamro Rent was built from real experience managing Nepali rental properties. Our story, mission, and the team behind the platform.",
      },
...
function AboutPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: isLoading ? "…" : data && data.tenants > 0 ? `${data.tenants}+` : t("about.growing"),
    ...
    },
    {
      ...
      value: isLoading
        ? "…"
        : data && data.paymentsNPR > 0
          ? fmtNPR(data.paymentsNPR)
          : t("about.growing"),
      label: t("about.stats.3"),
    },
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 pb-16 sm:pb-20">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((s) => (
            <div key={s.label} className="glass glass-hover rounded-2xl p-5 sm:p-6">
              <div className="text-primary mb-3">{s.icon}</div>
              <div
                className={`font-display text-2xl sm:text-3xl font-bold mb-1 transition-all ${isLoading ? "opacity-40" : ""}`}
              >
                {s.value}
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            <div>
              ...
              <div className="space-y-5 text-background/75 text-sm sm:text-base leading-relaxed">
                ...
                <p className="text-background font-semibold text-base sm:text-lg">
                  {t("about.story.pKey")}
                </p>
              </div>
            </div>
              ...
              {[
                {
                  ...
                  body: t("about.insight.body"),
                },
                {
                  icon: <Code2 className="h-5 w-5" />,
                  title: t("about.build.title"),
                  body: t("about.build.body"),
                },
                {
                ...
                <div
                  key={item.title}
                  className="flex gap-4 bg-background/5 rounded-2xl p-5 border border-background/10"
                >
                  <div className="h-10 w-10 rounded-xl bg-primary/20 text-primary flex items-center justify-center flex-shrink-0">
                    {item.icon}
                  </div>
                  <div>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        ...
        <h2 className="font-display text-3xl sm:text-4xl max-w-2xl leading-tight mb-10">
          {t("about.missionSec.title")}
        </h2>
        <div className="grid sm:grid-cols-2 gap-4">
          {[
            {
              title: "Bikram Sambat — not an afterthought",
              body: "Every bill, every payment date, every report uses BS. This isn't a Gregorian app with a conversion layer bolted on. BS is the foundation.",
            },
            {
              title: "The carry-forward problem — solved",
              body: "Tenant paid रू 9,500 but owed रू 10,000? Next month's bill automatically shows the रू 500 due. No manual tracking, no forgetting.",
            },
            {
              title: "NEA electricity, correctly calculated",
              body: "Enter previous and current meter readings. The app calculates units consumed, applies the rate, adds the service charge. You just read the meter.",
            },
            {
              title: "Receipts your tenants can actually open",
              body: "Every bill generates a public link. Tenants open it on WhatsApp, see a clean professional bill — no app download, no account needed.",
            },
            {
              title: "All your data, private to you",
              body: "Your tenant list, payment history, and bills are private to your account. No one else can see them. Export your full ledger anytime.",
            },
            {
              title: "Free — because it should be",
              body: "Small landlords shouldn't pay SaaS fees for basic billing. Hamro Rent is free to use. Professional tools shouldn't be for big businesses only.",
            },
          ].map((item) => (
            <Card key={item.title} className="glass glass-hover p-5 sm:p-6">
      ...
      <section className="border-y border-white/60">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
          ...
          <Card className="glass-deep p-6 sm:p-8 max-w-2xl">
            <div className="flex items-start gap-5">
              <div className="h-14 w-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center flex-shrink-0 font-display text-2xl">
                S
              </div>
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {["Full-stack development", "Nepal real estate", "Bikram Sambat systems"].map(
                    (tag) => (
                      <span
                        key={tag}
