  ...
  head: () => ({
    meta: [
      { title: "Features — Hamro Rent | Billing Tools for Nepali Landlords" },
      {
        name: "description",
        content:
          "Bikram Sambat bills, electricity auto-calculation, carry-forward balances, WhatsApp receipts, Excel export. Built for Nepal.",
      },
...
const FEATURE_GROUPS = [
  ...
  },
  {
    ...
    subhead:
      "Full profiles, smart defaults, instant bill history. Add a tenant once — bills fill themselves.",
  ...
  },
  {
    ...
    subhead:
      "Cash, eSewa, Khalti — record it all. Partial payments, overpayments, complete history.",
  ...
  },
  {
    ...
    subhead:
      "Your tenant gets a clean bill on WhatsApp. Your accountant gets a clean Excel. You get 5 minutes back.",
  ...
  },
  {
    ...
    subhead:
      "Row-level security means no one — including us — can read your tenant data without your login.",
...
function FeatureCard({
  ...
  f: (typeof FEATURE_GROUPS)[0]["features"][0];
  inView: boolean;
  delay: number;
}) {
  const Icon = f.icon;
  return (
    <div
      className={`glass glass-hover group relative rounded-2xl overflow-hidden ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
    ...
    >
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          {f.badge && (
            <span
              className={`text-[10px] font-bold uppercase tracking-wide px-2 py-1 rounded-full ${f.badgeColor}`}
            >
              {f.badge}
            </span>
          )}
          ...
          <div className="space-y-2">
            {f.demo.rows.map((row) => (
              <div key={row.key} className="flex items-center justify-between">
                ...
                <span
                  className={`text-xs font-semibold font-mono ${row.highlight ? "text-primary" : "text-foreground"}`}
                >
                  {row.value}
                </span>
              </div>
    ...
    <div className="relative min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative overflow-hidden pt-16 sm:pt-24 pb-12" ref={heroSection.ref}>
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          {/* Badge */}
          <div
            className={`transition-all duration-700 ${heroSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
          ...
          <h1
            className={`font-display text-4xl sm:text-5xl lg:text-6xl leading-[1.05] max-w-3xl mb-5 transition-all duration-700 delay-100 ${heroSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            Every feature you need — <span className="text-primary">none you don't.</span>
          </h1>

          <p
            className={`text-base sm:text-xl text-muted-foreground max-w-2xl leading-relaxed mb-10 transition-all duration-700 delay-200 ${heroSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            Hamro Rent is designed around the real workflow of a Nepali landlord — BS calendar, NEA
            electricity, eSewa payments, WhatsApp receipts. Nothing generic. Nothing bloated.
          </p>

          {/* Stats row */}
          <div
            className={`flex flex-wrap gap-8 sm:gap-12 mb-12 transition-all duration-700 delay-300 ${heroSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            {[
          ...
          {/* Group nav pills */}
          <div
            className={`flex flex-wrap gap-2 transition-all duration-700 delay-400 ${heroSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
        ...
        {FEATURE_GROUPS.map((group, gi) => {
          ...
          return (
            <section key={group.id} id={group.id} ref={ref} className="scroll-mt-24">
              {/* Group header */}
              <div
                className={`flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              >
                <div>
                  <div
                    className={`inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-4 ${group.iconBg} border border-current/10`}
                  >
                    {group.label}
                  </div>
      ...
      {/* ══ COMPARISON STRIP ═════════════════════════════════════════════════ */}
      <section className="border-y border-white/60">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left pb-3 font-medium text-muted-foreground w-1/3">
                    Feature
                  </th>
                  <th className="pb-3 font-semibold text-center w-1/4">
                    <span className="text-primary">Hamro Rent</span>
                  </th>
                  <th className="pb-3 font-medium text-muted-foreground text-center w-1/4">
                    Notebook / WhatsApp
                  </th>
                  <th className="pb-3 font-medium text-muted-foreground text-center w-1/4">
                    Generic apps
      ...
      <section className="max-w-5xl mx-auto px-5 sm:px-8 py-16 sm:py-20">
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={stat} className="glass glass-hover flex gap-4 items-start p-5 rounded-2xl">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-10">
              <div className="max-w-xl">
                ...
                <p className="text-background/70 leading-relaxed">
                  Join Nepali landlords who've replaced their rent notebooks with Hamro Rent. No
                  card, no install, no nonsense.
                </p>
              </div>
              <div className="flex flex-col gap-3 flex-shrink-0">
                <Link
                  to="/login"
                  className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground text-sm font-semibold px-8 py-3.5 rounded-full hover:bg-primary/90 transition-all shadow-xl shadow-primary/30 hover:-translate-y-0.5"
                >
                  Get started free <ArrowRight className="h-4 w-4" />
                </Link>
                <Link
                  to="/pricing"
                  className="inline-flex items-center justify-center gap-2 border border-background/20 text-background text-sm font-medium px-8 py-3.5 rounded-full hover:bg-background/10 transition-all"
                >
